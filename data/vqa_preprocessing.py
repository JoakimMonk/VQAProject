...
def main(params):
    ...
    train = []
    test = []
    imdir='train_images/abstract_v002_%s_%012d.jpg'
    ...
    if params['split'] == 3:
        ...
        print 'Loading annotations and questions...'
        train_anno = json.load(open('train_annotations.json', 'r'))
        #val_anno = json.load(open('annotations/mscoco_val2014_annotations.json', 'r'))

        train_ques = json.load(open('train_questions/MultipleChoice_abstract_v002_train2015_questions.json', 'r'))
        val_ques = json.load(open('test_questions/MultipleChoice_abstract_v002_val2015_questions.json', 'r'))

        subtype = 'train2015'
        for i in range(len(train_anno)):
            ans = train_anno[i]['multiple_choice_answer']
            question_id = train_anno[i]['question_id']
            image_path = imdir%(subtype, train_anno[i]['image_id'])
        ...
        subtype = 'val2015'
        for i in range(len(val_ques['questions'])):
            #ans = val_anno['annotations'][i]['multiple_choice_answer']
            question_id = val_ques['questions'][i]['question_id']
            image_path = imdir%(subtype, val_ques['questions'][i]['image_id'])
